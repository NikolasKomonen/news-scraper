...
class WayBackMachineArticleRetriever:
    def __call__(self, url: URL) -> ArticleText:
        ...
        assert (
            actual_retriever is not None
        ), "URL given was not from the wayback machine"
        ...
        ), f"No articel retriever can be resolved for {url.human_repr()}"

        if article_text.metadata.url != original_article_url:
            metadata = DefaultArticleMetadata(
                datetime=article_text.metadata.datetime, url=original_article_url
            )
